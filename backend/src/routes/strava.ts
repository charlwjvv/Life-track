...
stravaRouter.post('/token', async (req: AuthRequest, res: Response) => {
  try {
    const { code } = req.body;
    ...
    if (!response.ok) return res.status(400).json({ error: data.message || 'Failed to exchange token' });
...
stravaRouter.get('/activities', async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!response.ok) { const err = (await response.json()) as any; return res.status(400).json({ error: err.message }); }
...
stravaRouter.get('/weekly', async (req: AuthRequest, res: Response) => {
  try {
    const now = new Date();
...
stravaRouter.get('/routes', async (req: AuthRequest, res: Response) => {
  try {
