...
function parseRanking(
  text: string,
  labelToMember: Record<string, Stage1Result>
): RankingEntry[] {
...
function calculateAggregate(s2: Stage2Result[]): AggregateRanking[] {
  const totals: Record<string, { sum: number; count: number }> = {};
  for (const result of s2) {
    for (const entry of result.parsedRanking) {
      if (!totals[entry.memberId]) totals[entry.memberId] = { sum: 0, count: 0 };
...
async function stage1(query: string): Promise<Stage1Result[]> {
  const results = await Promise.allSettled(
    COUNCIL_MEMBERS.map((m) =>
      spawnAgent(m.persona, query).then((r) => ({
        memberId: m.id,
        memberName: m.name,
        content: r.text,
      }))
  ...
  return results
    .filter((r): r is PromiseFulfilledResult<Stage1Result> => r.status === 'fulfilled')
    .map((r) => r.value);
}
...
async function stage2(
  ...
  const labelToMember: Record<string, Stage1Result> = {};
  for (let i = 0; i < s1.length; i++) labelToMember[labels[i]] = s1[i];
  ...
  const evalText = (await spawnAgent(EVALUATOR.persona, evalPrompt)).text;
...
async function stage3(query: string, s1: Stage1Result[], aggregate: AggregateRanking[]): Promise<string> {
  ...
  return (await spawnAgent(CHAIRMAN.persona, prompt)).text;
}
...
async function runCouncil(query: string): Promise<CouncilOutput> {
  const s1 = await stage1(query);
  if (s1.length === 0) throw new Error('No responses from council');

  const labels = s1.map((_, i) => String.fromCharCode(65 + i));
  const labelToModel: Record<string, string> = {};
  for (let i = 0; i < s1.length; i++) labelToModel[labels[i]] = s1[i].memberName;
